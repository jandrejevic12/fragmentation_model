...
ax1.plot(x, A*x, linestyle='dashed', lw=3, color='k')
ax2.plot(2*x, 2*x, linestyle='dashed', lw=3, color='k')
t = np.logspace(0,2.5,100)
a = np.sqrt(t/tau)
ax3.plot(t, a*(a+1)/t**2, linestyle='dashed', lw=3, color='k')
...
for i,(exp,delta) in enumerate(zip(Exps,Deltas)):
    for t in range(len(Areas[exp])):
        if t<3 or exp in [45, 43, 41]: 
            ...
            ax1.scatter(tt/(a+1)*(1-delta), per*(1-delta), edgecolor=list(cmap(cnorm(delta)))[:-1]+[0.9], marker=markers[t], s=150, lw=4, facecolor='w', zorder=10)
            ax2.scatter(ell, (1-delta)*A*tt/(a+1), edgecolor=list(cmap(cnorm(delta)))[:-1]+[0.9], marker=markers[t], s=150, lw=4, facecolor='w', zorder=10)
            ax3.scatter(tt, s, edgecolor=list(cmap(cnorm(delta)))[:-1]+[0.9], marker=markers[t], s=150, lw=4, facecolor='w', zorder=10)
