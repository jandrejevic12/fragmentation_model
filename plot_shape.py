...
# fitted a vs t
fig, axes, cbax = fig_with_cbar((8,7), 1)
ax = axes[0]
markers = ['o','s','^','v']
tt = np.linspace(0,250)
ax.plot(tt, np.sqrt(tt/tau), linestyle='dashed', lw=3, color='k')
...
for i,(exp,delta) in enumerate(zip(Exps,Deltas)):
    ...
    for t in range(len(Areas[exp])):    
        ...
        tt = np.sqrt(g*(g+1)/s)
        ax.scatter(tt, g, edgecolor=list(cmap(cnorm(delta)))[:-1]+[0.9], marker=markers[t], s=150, lw=4, facecolor='w', zorder=10)
