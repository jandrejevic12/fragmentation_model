...
hs = h/ds/np.sum(h)
ax.scatter(sl, hs, edgecolor=list(cmap(cnorm(delta)))[:-1]+[0.7], marker='o', s=150, lw=3, facecolor=list(cmap(cnorm(delta)))[:-1]+[0.4], zorder=10)
...
ax.plot(sl, y, color='royalblue', alpha=0.6, lw=5, label='best fit')
...
ax.plot(sl, y, color='royalblue', alpha=0.6, lw=5, label='universal')
